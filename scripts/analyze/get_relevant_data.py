...
heldout_featurefile='/mount/arbeitsdaten14/projekte/sfb-732/d8/falensaa/BiasInArguments/intermediate-data/tal_elal_2016/feature_sets/heldout_OP.csv'
...
heldout_subset, heldout_feature_df = load_dataframes(heldout_datafile, heldout_featurefile)
...
comment_info = pd.DataFrame(columns=['id', 'perc_explicit_gender_in_comments', 'perc_explicit_gender_in_comments_m', 'perc_explicit_gender_in_comments_f', 'perc_author_gender_in_comments_m', 'perc_author_gender_in_comments_f', 'perc_author_gender_in_comments', 'avg_comment_score', 'avg_comment_deltas'])
for _, row in subset.iterrows():
    comments_df = pd.json_normalize(row['comments'])
    if comments_df.empty:
        new_entry = {'id':row['id'], 'perc_explicit_gender_in_comments':0.0, 'perc_explicit_gender_in_comments_m':0.0, 'perc_explicit_gender_in_comments_f':0.0, 'perc_author_gender_in_comments_m':0.0, 'perc_author_gender_in_comments_f':0.0, 'perc_author_gender_in_comments':0.0, 'avg_comment_score':0.0, 'avg_comment_deltas':0.0}    
    else:
        ...
        comments_df['author_flair_text'] = comments_df['author_flair_text'].astype(str)
        comments_df['author_flair_text'] = pd.to_numeric(comments_df['author_flair_text'].str[:-1], errors='coerce')
        avg_comment_deltas = comments_df['author_flair_text'].sum() / comment_count
        new_entry = {'id':row['id'], 'perc_explicit_gender_in_comments':perc_explicit_mentions, 'perc_explicit_gender_in_comments_m':explicit_gender_m, 'perc_explicit_gender_in_comments_f':explicit_gender_f, 'perc_author_gender_in_comments_m':author_gender_m, 'perc_author_gender_in_comments_f':author_gender_f, 'perc_author_gender_in_comments':perc_author_mentions, 'avg_comment_score':avg_comment_score, 'avg_comment_deltas': avg_comment_deltas}
...
subset['author_flair_text'] = subset['author_flair_text'].astype(str)
subset['deltas'] = pd.to_numeric(subset['author_flair_text'].str[:-1], errors='coerce')

df = pd.merge(df_acl2022_tan2016, subset[['id','author_gender', 'explicit_gender', 'num_comments', 'score', 'edited_binary', 'topic', 'deltas']], on='id', how='left')
